...
from src.piper_coordinator import PiperProgramCoordinator, create_piper_placement_group
from src.piper_compile import piper_setup
from src.piper import piper_exec_dag
from src.piper_schedule import load_schedule_directives
from src.piper_utils import piper_metadata, create_logger, LOG_LEVEL
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="test/schedules/qwen_pp_only.json",
...
if __name__ == "__main__":
    ...
    args = parse_args()
    args.num_stages = _derive_num_stages(args.schedule_directives_file)
    logger.info(args)
    if args.address != "":
        ray.init(
    ...
    if args.metrics_json_out:
        os.makedirs(os.path.dirname(args.metrics_json_out) or ".", exist_ok=True)
        with open(args.metrics_json_out, "w", encoding="utf-8") as f:
            json.dump(dp_metrics, f)
        logger.info(f"Raw metrics written to {args.metrics_json_out}")
    ray.shutdown()
